...
FBC fragments are stored in the `redhat-user-workloads` Quay namespace by default. The visibility of the Quay repo can be private as long as you are using the default namespace. If you decide to house them elsewhere in a private organization, it is mandatory for the repo to be **public** at the time of the release.
